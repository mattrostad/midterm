...
const editTask = function () {
  ...
  console.log('edit task: getting taskstab info', taskData, taskElm)
  $('#mytaskstab').on('show.bs.taskstab', function (event) {
    const taskstab = $(this);
    taskstab.find('#checkbox').prop('checked', (taskData.completed_date) ? true : false);
    taskstab.find('#title').val(taskData.task_name);
    taskstab.find('#category').val(taskData.category_name);
    taskstab.find('.datepicker').val(taskData.scheduled_date ? new Date(taskData.scheduled_date).toLocaleDateString() : '');
    taskstab.find('#priority').val(taskData.priority);
    taskstab.off('show.bs.taskstab');
  });

  $('#mytaskstab').data('taskId', taskData.id); // Attach task id if we are editing
  $("#newTask").click();
};
...
$(document).ready(function () {
  ...
  // Clear taskstab form on click close taskstab button
  $('#mytaskstab').on('hidden.bs.taskstab', function (e) {
    const taskstab = $(this);
    taskstab.find('#checkbox').prop('checked', false);
    taskstab.find('#title').val('');
    taskstab.find('#category').val('');
    taskstab.find('.datepicker').val('');
    taskstab.find('#priority').val('');

    // Need to removed attached data for next call
    $('#mytaskstab').removeData('taskId');
  });
